...
func TestParseFileLicense2(t *testing.T) {
	...
	if err != nil {
		t.Errorf("读取coder-publickey.pem文件失败: %v", err)
		return
	}
	...
	licenseJwtContent, err := os.ReadFile("/lyndon/iData/coder/keys/license.jwt")
	if err != nil {
		t.Errorf("读取license.jwt文件失败: %v", err)
		return
	}
	licenseString := string(licenseJwtContent)
...
func TestSlog(t *testing.T) {
	...
	logger.Debug("This is a debug message", slog.String("key1", "value1"))
	logger.Info("This is an info message", slog.Int("key2", 42))
	logger.Warn("This is a warning message", slog.Float64("key3", 3.14))
	logger.Error("This is an error message", slog.Bool("key4", true))

	logger.Info("Structured log message", slog.String("id", "1234"))
}
